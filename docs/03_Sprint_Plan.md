...
**Execution notes (2026-08-22):**
 - Tier 1 tester: `sentinel/backend/app/testers/career_os.py`, registered under slug `Resmaker` (matching the Sentinel dashboard project name — `_slug` is case-preserving) with `auto_launch=False` and `ports=(8000,)`. POST assertions create a uniquely-tokened knowledge item via the API, run `/search/`, `/build/suggest`, `/build/resume`, `/validate/` against it, then DELETE it and prove the 404 — the real corpus is untouched. Live-verified end-to-end against the real backend on a scratch port before handoff.
